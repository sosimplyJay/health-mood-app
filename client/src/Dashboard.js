...
function Dashboard({ name, email, onLogout }) {
  ...
  // Initial data loading and countdown setup
  useEffect(() => {
    fetchMealLogs();
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('/analyze-meal', {
      ...
      .catch((error) => {
        setMessage('Error logging meal: ' + (error.response?.data?.message || error.message));
      });
  };

  const fetchMealLogs = () => {
    axios.get(`/meal-logs?email=${email}`)
      .then(({ data }) => {
        setMealLogs(data);
      })
      .catch((error) => {
  ...
  const renderTrendsSection = () => (
    <div className="dashboard-section">
      ...
      {mealLogs.length > 0 ? (
        ...
        <p>No meal data yet to display a chart.</p>
      )}
    </div>
  );
        ...
        <table className="meal-table">
          ...
          <tbody>
            {mealLogs.length > 0 ? (
              ...
              <tr>
                <td colSpan="2">No meal logs found. Add your first meal!</td>
              </tr>
            )}
          </tbody>
        </table>
  ...
  const renderTimingSection = () => (
    <div className="dashboard-section">
      ...
      <p style={{ marginTop: '1rem' }}>
        <em>Choose a plan that fits your routine and supports your energy throughout the day!</em>
      </p>
    </div>
  );
  ...
  return (
    <div className={`dashboard fade-transition ${moodBackground}`}>
      ...
      <button className="sand-timer-btn" onClick={startSandTimer}>
        Start 5-Minute Meal Timer ⏳
      </button>
      
      {timerActive && timerSeconds > 0 && (
        <p className="sand-timer-display">
          <img 
            src="https://upload.wikimedia.org/wikipedia/commons/5/55/Simple_icon_hourglass.svg" 
            alt="Hourglass icon" 
            style={{ width: '28px', marginRight: '8px', verticalAlign: 'middle' }} 
          />
      ...
      <div className="dashboard-nav">
        ...
        <button 
          className={currentPage === 'timing' ? 'active' : ''} 
      ...
      {currentPage === 'log' && renderLogSection()}
      {currentPage === 'trends' && renderTrendsSection()}
      {currentPage === 'history' && renderHistorySection()}
      {currentPage === 'summary' && renderSummarySection()}
      {currentPage === 'timing' && renderTimingSection()}
